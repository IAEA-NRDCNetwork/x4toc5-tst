...
def readC5file(fileName):
    print('---readC5file---'+fileName)
    ff=open(fileName,"r")
    ii=0
    datasets=[]
    nowDataset=None
...
#        elif line1.startswith("#C5CORR "):   ds['corrected']='CORRECTED'
        ...
        elif line1.startswith("#ORIG_REACTION "): ds['OrigReaction']=subs[1]
        elif line1.startswith("#DERIVED "):  ds['derived']=int(subs[1])
 ...
 #                   print('C5DATA_started:'+str(C5DATA_started)+' ii='+str(ii)+' C5DATA_iiline='+str(C5DATA_iiline)+'\t['+line1+']')
    ...
    if corrected is not None:
        if ds['author1'] is not None: ds['author1']+='*'
        else: ds['author1']='*'
    if ds['derived'] is not None:
        if ds['derived'] ==10 :
            if ds['author1'] is not None: ds['author1']+='^'
            else: ds['author1']='*'
